...
const getStatusBadgeClasses = (status: string) => {
  ...
  switch (status) {
    ...
    default:                return `${base} bg-[rgba(42,36,30,0.05)] text-tide-ink/70 border-[rgba(60,50,35,0.12)]`;
  }
};
...
const getEffortBadgeClasses = (effort?: string) => {
  ...
  switch (effort) {
    ...
    case 'M':  return `${base} bg-[rgba(90,138,171,0.10)] text-tide-blue-deep border-blue-200`;
    ...
    default:   return `${base} bg-[rgba(252,247,238,0.55)] text-tide-ink/55 border-[rgba(60,50,35,0.12)]`;
  }
};
...
const getCategoryBadgeClasses = (cat?: string) => {
  ...
  switch (cat) {
    ...
    case 'OTHER':        return `${base} bg-[rgba(252,247,238,0.55)] text-tide-ink/70 border-[rgba(60,50,35,0.12)]`;
    default:             return `${base} bg-[rgba(252,247,238,0.55)] text-tide-ink/40 border-[rgba(42,36,30,0.07)]`;
  }
};
  ...
  <div class="min-h-screen bg-[rgba(252,247,238,0.55)]">
    ...
    <div class="px-6 py-6 space-y-6">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <div class="bg-[rgba(255,253,247,0.92)] rounded-xl border border-[rgba(60,50,35,0.12)] shadow-sm p-5">
          <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">Total</p>
          <p class="mt-2 text-3xl font-bold text-tide-ink">{{ stats.total }}</p>
        </div>
        <div class="bg-[rgba(255,253,247,0.92)] rounded-xl border border-green-200 shadow-sm p-5">
        ...
        <div class="bg-[rgba(255,253,247,0.92)] rounded-xl border border-blue-200 shadow-sm p-5">
          <p class="text-xs font-medium text-tide-blue-deep uppercase tracking-wide">Approved</p>
          <p class="mt-2 text-3xl font-bold text-tide-blue-deep">{{ stats.approved }}</p>
        </div>
        <div class="bg-[rgba(255,253,247,0.92)] rounded-xl border border-indigo-200 shadow-sm p-5">
      ...
      <!-- Table Card -->
      <div class="bg-[rgba(255,253,247,0.92)] shadow-sm rounded-xl border border-[rgba(60,50,35,0.12)] overflow-hidden">

        <!-- Filter Tabs -->
        <div class="border-b border-[rgba(60,50,35,0.12)] px-4 overflow-x-auto">
          <nav class="flex gap-1 -mb-px" aria-label="Tabs">
            <button
              ...
              :class="
                activeTab === tab.value
                  ? 'border-blue-600 text-tide-blue-deep'
            ...
            >
              ...
              <span
                ...
                :class="activeTab === tab.value ? 'bg-blue-100 text-tide-blue-deep' : 'bg-[rgba(42,36,30,0.05)] text-tide-ink/70'"
              >
        ...
        <!-- Secondary Filters -->
        <div class="px-4 py-3 border-b border-[rgba(42,36,30,0.07)] flex flex-wrap gap-3 bg-[rgba(252,247,238,0.55)]">
          <!-- Category filter -->
          <div class="flex items-center gap-1.5">
            <Tag class="h-3.5 w-3.5 text-tide-ink/40 flex-shrink-0" />
            <select
              v-model="filterCategory"
              class="text-sm border border-[rgba(60,50,35,0.16)] rounded-lg py-1.5 pl-2.5 pr-7 bg-[rgba(255,253,247,0.92)] focus:outline-none focus:ring-2 focus:ring-tide-blue focus:border-tide-blue"
            >
          ...
          <!-- Milestone filter -->
          <div class="flex items-center gap-1.5">
            <Flag class="h-3.5 w-3.5 text-tide-ink/40 flex-shrink-0" />
            <input
              v-model="filterMilestone"
              type="text"
              placeholder="Filter by milestone..."
              list="milestone-list"
              class="text-sm border border-[rgba(60,50,35,0.16)] rounded-lg py-1.5 px-2.5 bg-[rgba(255,253,247,0.92)] focus:outline-none focus:ring-2 focus:ring-tide-blue focus:border-tide-blue w-40"
            />
          ...
          <!-- Assignee filter -->
          <div class="flex items-center gap-1.5">
            <User class="h-3.5 w-3.5 text-tide-ink/40 flex-shrink-0" />
            <input
          ...
          <button
            ...
            class="inline-flex items-center gap-1 text-xs text-tide-ink/55 hover:text-tide-ink/80 px-2 py-1.5 rounded-lg hover:bg-[rgba(42,36,30,0.05)] transition-colors"
          >
        ...
        <!-- Loading skeleton -->
        <div v-if="isLoading" class="divide-y divide-[rgba(42,36,30,0.06)]">
        ...
        <div v-else-if="filteredRequests.length" class="overflow-x-auto">
          <table class="min-w-full divide-y divide-[rgba(42,36,30,0.06)]">
            <thead class="bg-[rgba(252,247,238,0.55)]">
              <tr>
                <th class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider">Ticket #</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider">Title</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider">Category</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider">Assigned To</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider">Status</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider">Priority</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider">Effort</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider">Milestone</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider">Due Date</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider">Created At</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider sr-only">Actions</th>
              </tr>
            </thead>
            <tbody class="bg-[rgba(255,253,247,0.92)] divide-y divide-[rgba(42,36,30,0.06)]">
              <tr
                ...
                :key="req.id"
                class="hover:bg-[rgba(252,247,238,0.55)] transition-colors cursor-pointer"
              ...
              >
                ...
                <td class="px-4 py-4">
                  <span
                    ...
                    class="font-mono text-xs font-bold bg-[rgba(90,138,171,0.10)] text-tide-blue-deep border border-blue-200 px-2 py-0.5 rounded-full whitespace-nowrap"
                  >
                    {{ req.ticketNumber }}
                  </span>
                  <span v-else class="text-xs text-tide-ink/40 font-mono">—</span>
                </td>

                <!-- Title -->
                <td class="px-4 py-4">
                  <p class="text-sm font-medium text-tide-ink truncate max-w-xs">{{ req.title }}</p>
                  <p v-if="req.tags?.length" class="text-xs text-tide-ink/40 mt-0.5 truncate">
                    {{ req.tags.join(', ') }}
                  </p>
                </td>
                ...
                <td class="px-4 py-4">
                  ...
                  <span v-else class="text-xs text-tide-ink/40">—</span>
                </td>
                ...
                <td class="px-4 py-4">
                  <div v-if="req.assignedTo" class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-tide-ink/70 truncate max-w-[80px]">{{ req.assignedTo }}</span>
                  </div>
                  <span v-else class="text-xs text-tide-ink/40">—</span>
                </td>
                ...
                <td class="px-4 py-4">
                  <div v-if="req.priority !== undefined" class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-tide-ink/55 tabular-nums">{{ req.priority }}</span>
                  </div>
                  <span v-else class="text-xs text-tide-ink/40">—</span>
                </td>
                ...
                <td class="px-4 py-4">
                  <span
                    v-if="req.milestone"
                    class="inline-flex items-center px-2 py-0.5 rounded text-xs bg-[rgba(42,36,30,0.05)] text-tide-ink/70 border border-[rgba(60,50,35,0.12)]"
                  >
                    {{ req.milestone }}
                  </span>
                  <span v-else class="text-xs text-tide-ink/40">—</span>
                ...
                <!-- Created At -->
                <td class="px-4 py-4">
                  <span class="text-sm text-tide-ink/55 whitespace-nowrap">{{ formatDate(req.createdAt) }}</span>
                </td>
                ...
                <td class="px-4 py-4" @click.stop>
                  <button
                    @click="openDetail(req)"
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 border border-[rgba(60,50,35,0.12)] rounded-lg text-xs font-medium text-tide-ink/70 hover:bg-[rgba(252,247,238,0.55)] hover:text-tide-blue-deep hover:border-blue-200 transition-colors"
                  >
        ...
        <div v-else class="text-center py-16">
          ...
          <h3 class="text-sm font-medium text-tide-ink">No feature requests</h3>
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-[rgba(255,253,247,0.92)] rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col overflow-hidden">

          <!-- Modal Header -->
          <div class="flex items-start justify-between p-5 border-b border-[rgba(42,36,30,0.07)] gap-4 bg-[rgba(255,253,247,0.92)]">
            <div class="flex items-center gap-3 min-w-0">
              <!-- Large ticket number badge -->
              <span
                v-if="selectedRequest.ticketNumber"
                class="font-mono text-sm font-bold bg-[rgba(90,138,171,0.10)] text-tide-blue-deep border border-blue-200 px-3 py-1 rounded-full whitespace-nowrap flex-shrink-0"
              >
                {{ selectedRequest.ticketNumber }}
              </span>
              <div class="min-w-0">
                <h3 class="text-lg font-semibold text-tide-ink leading-snug truncate">{{ selectedRequest.title }}</h3>
                <div class="flex items-center gap-2 mt-1 flex-wrap">
                  ...
                  <span v-if="selectedRequest.createdBy" class="text-xs text-tide-ink/40">by {{ selectedRequest.createdBy }}</span>
                  <span v-if="selectedRequest.createdAt" class="text-xs text-tide-ink/40">· {{ formatDate(selectedRequest.createdAt) }}</span>
                </div>
              </div>
            </div>
            <button
              @click="closeDetail"
              class="p-2 text-tide-ink/40 hover:text-tide-ink/70 hover:bg-[rgba(42,36,30,0.05)] rounded-lg transition-colors flex-shrink-0"
            >
              <X class="h-5 w-5" />
            </button>
          </div>
          ...
          <div class="flex flex-col lg:flex-row flex-1 overflow-hidden min-h-0">
            ...
            <div class="flex-1 overflow-y-auto border-b lg:border-b-0 lg:border-r border-[rgba(42,36,30,0.07)] divide-y divide-[rgba(42,36,30,0.06)]">

              <!-- Description + Summary -->
              <div class="p-5 space-y-4">
                <div>
                  <p class="text-xs font-semibold text-tide-ink/55 uppercase tracking-wide mb-1.5">Description</p>
                  <p class="text-sm text-tide-ink/80 leading-relaxed whitespace-pre-wrap">{{ selectedRequest.description || '—' }}</p>
                </div>

                <div v-if="selectedRequest.structuredSummaryData || selectedRequest.structuredSummary">
                  <p class="text-xs font-semibold text-tide-ink/55 uppercase tracking-wide mb-1.5">AI Summary</p>
                  ...
                  >
                    ...
                    <p class="text-sm text-tide-ink/80 leading-relaxed whitespace-pre-wrap">{{ selectedRequest.structuredSummary }}</p>
                  </div>
                </div>
              </div>
              ...
              <div v-if="selectedRequest.conversation?.length" class="p-5 space-y-3">
                <p class="text-xs font-semibold text-tide-ink/55 uppercase tracking-wide mb-3 flex items-center gap-1.5">
                ...
                >
                  <div v-if="msg.role === 'assistant'" class="flex items-start gap-2 max-w-[85%]">
                    ...
                    <div class="bg-gradient-to-br from-slate-50 to-blue-50 border border-blue-100 rounded-xl rounded-tl-sm px-3.5 py-2.5">
                      ...
                      <p v-if="msg.timestamp" class="text-xs text-tide-ink/40 mt-1">
            ...
            <div class="w-full lg:w-80 xl:w-96 flex-shrink-0 overflow-y-auto divide-y divide-[rgba(42,36,30,0.06)]">
              ...
              <div class="p-5 space-y-4">
                <p class="text-xs font-semibold text-tide-ink/55 uppercase tracking-wide">Properties</p>

                <!-- Category -->
                <div>
                  <label class="block text-xs font-medium text-tide-ink/70 mb-1.5">Category</label>
                  <select
                    v-model="actionCategory"
                    class="block w-full border border-[rgba(60,50,35,0.16)] rounded-lg py-2 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-tide-blue focus:border-tide-blue"
                  >
                ...
                <!-- Priority slider -->
                <div>
                  <label class="block text-xs font-medium text-tide-ink/70 mb-1.5">
                    Priority
                    <span class="ml-1.5 text-tide-blue-deep font-bold tabular-nums">{{ actionPriority }}</span>
                  </label>
                  <input
                  ...
                  />
                  <div class="flex justify-between text-xs text-tide-ink/40 mt-1">
                    <span>0 – Low</span>
                ...
                <!-- Effort buttons -->
                <div>
                  <label class="block text-xs font-medium text-tide-ink/70 mb-1.5">Estimated Effort</label>
                  <div class="flex gap-2">
                    <button
                      ...
                      :class="
                        actionEffort === opt
                          ? 'border-tide-blue bg-[rgba(90,138,171,0.10)] text-tide-blue-deep'
                          : 'border-[rgba(60,50,35,0.12)] text-tide-ink/55 hover:border-[rgba(60,50,35,0.16)] hover:bg-[rgba(252,247,238,0.55)]'
                      "
                    >
                      {{ opt }}
                    </button>
                ...
                <!-- Assigned To -->
                <div>
                  <label class="block text-xs font-medium text-tide-ink/70 mb-1.5">Assigned To</label>
                  <div class="flex gap-2">
                    <input
                      v-model="actionAssignedTo"
                      type="text"
                      placeholder="e.g. john.doe"
                      class="flex-1 border border-[rgba(60,50,35,0.16)] rounded-lg py-2 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-tide-blue focus:border-tide-blue min-w-0"
                    />
                    <button
                      @click="handleAssign"
                ...
                <!-- Milestone -->
                <div>
                  <label class="block text-xs font-medium text-tide-ink/70 mb-1.5">Milestone</label>
                  <input
              ...
              <div class="p-5 space-y-3">
                <p class="text-xs font-semibold text-tide-ink/55 uppercase tracking-wide">Actions</p>
                ...
                <div class="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    :disabled="isActioning"
                    class="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium text-white bg-tide-blue-btn-deep hover:bg-tide-blue-deep disabled:opacity-50 transition-colors shadow-sm"
                  >
                  ...
                  <button
                    ...
                    class="inline-flex items-center gap-1 px-3 py-2 rounded-lg text-xs font-medium text-tide-ink/70 bg-[rgba(42,36,30,0.05)] hover:bg-gray-200 transition-colors"
                  >
                    <X class="w-3.5 h-3.5" /> Cancel
                  </button>
                </div>

                <div v-if="isActioning" class="flex items-center gap-2 text-xs text-tide-ink/55">
              ...
              <div class="p-5 space-y-3">
                <p class="text-xs font-semibold text-tide-ink/55 uppercase tracking-wide flex items-center gap-1.5">
                ...
                >
                  <div
                    ...
                    class="bg-[rgba(252,247,238,0.55)] rounded-lg p-3 border border-[rgba(42,36,30,0.07)]"
                  >
                    <div class="flex items-center gap-2 mb-1">
                      ...
                      <span class="text-xs font-medium text-tide-ink/80">{{ comment.authorId || 'Admin' }}</span>
                      <span class="text-[10px] text-tide-ink/40 ml-auto">{{ formatDate(comment.createdAt) }}</span>
                    </div>
                    <p class="text-xs text-tide-ink/70 leading-relaxed">{{ comment.content }}</p>
                  </div>
                </div>
                <p v-else class="text-xs text-tide-ink/40">No internal comments yet.</p>
                ...
                <div class="space-y-2">
                  <textarea
                    ...
                    class="block w-full border border-[rgba(60,50,35,0.16)] rounded-lg py-2 px-3 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-tide-blue focus:border-tide-blue resize-none"
                  ></textarea>
                  <button
                ...
                >
                  ...
                  >
                    ...
                    <div class="flex-1 min-w-0">
                      <p class="text-xs text-tide-ink/80 leading-snug">
                        <span v-if="entry.fromStatus" class="text-tide-ink/55">
                          <span class="font-medium text-tide-ink/80">{{ entry.fromStatus.replace(/_/g, ' ') }}</span>
                          →
                        </span>
                        <span class="font-semibold text-tide-ink">{{ entry.toStatus.replace(/_/g, ' ') }}</span>
                        <span v-if="entry.changedBy" class="text-tide-ink/55"> by {{ entry.changedBy }}</span>
                      </p>
                      <p v-if="entry.changedAt" class="text-[10px] text-tide-ink/40 mt-0.5">{{ formatDateTime(entry.changedAt) }}</p>
                      <p v-if="entry.reason" class="text-[11px] text-tide-ink/55 mt-0.5 italic">{{ entry.reason }}</p>
                    </div>
                  </div>
                </div>
                <p v-else class="text-xs text-tide-ink/40">No status history yet.</p>
              </div>
            </div>
          </div>

          <!-- Modal Footer -->
          <div class="px-6 py-4 bg-[rgba(252,247,238,0.55)] border-t border-[rgba(42,36,30,0.07)] flex justify-end">
            <button
              @click="closeDetail"
              class="px-4 py-2 border border-[rgba(60,50,35,0.16)] rounded-lg text-sm font-medium text-tide-ink/80 hover:bg-[rgba(42,36,30,0.05)] transition-colors"
            >
              Close
            </button>
          </div>
