...
<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h3 class="text-lg font-semibold text-tide-ink">{{ t('itemLifecycleView.title') }}</h3>
      ...
      <div class="flex items-center space-x-3">
        <button
          @click="refresh"
          :disabled="loading"
          class="inline-flex items-center px-3 py-1.5 text-sm text-tide-ink/70 hover:text-tide-ink disabled:opacity-50"
    ...
    <div class="flex items-center space-x-4 pb-4 border-b border-[rgba(60,50,35,0.12)]">
      <div class="flex items-center space-x-2">
        <Filter class="h-4 w-4 text-tide-ink/40" />
        <select 
      ...
      <div class="text-sm text-tide-ink/55">
        ...
        <button
          ...
          class="ml-2 text-tide-blue-deep hover:text-blue-800 hover:underline"
    ...
    <div v-if="loading" class="text-center py-8">
      <RefreshCw class="h-6 w-6 animate-spin mx-auto text-tide-ink/40 mb-2" />
      <p class="text-sm text-tide-ink/55">{{ t('itemLifecycleView.loading') }}</p>
    </div>

    <!-- Empty state -->
    <div v-else-if="!filteredLifecycles.length" class="text-center py-8">
      <Calendar class="h-12 w-12 mx-auto text-tide-ink/40 mb-4" />
      ...
      <button
        v-if="statusFilter"
        @click="statusFilter = ''"
        class="mt-2 text-sm text-tide-blue-deep hover:underline"
      >
    ...
    <!-- Lifecycle table -->
    <div v-else class="bg-[rgba(255,253,247,0.92)] border border-[rgba(60,50,35,0.12)] rounded-lg overflow-hidden">
        ...
        <tbody class="bg-[rgba(255,253,247,0.92)] divide-y divide-[rgba(42,36,30,0.08)]">
          <template v-for="lifecycle in filteredLifecycles" :key="lifecycle.id">
            ...
            <tr class="hover:bg-[rgba(252,247,238,0.55)]">
              ...
              <td class="px-6 py-3 whitespace-nowrap text-sm text-tide-ink">
                {{ formatDate(lifecycle.startTime) }}
              </td>
              <td class="px-6 py-3 whitespace-nowrap text-sm text-tide-ink">
                {{ formatDate(lifecycle.endTime) }}
              </td>
              <td class="px-6 py-3 whitespace-nowrap text-sm text-tide-ink/55">
            ...
            <tr v-if="expandedRows.has(lifecycle.id!) && lifecycle.events?.length">
              <td colspan="7" class="px-4 py-3 bg-[rgba(252,247,238,0.55)]">
                <div class="space-y-2 max-h-48 overflow-y-auto">
                  <div class="text-xs font-medium text-tide-ink/55 mb-3">
                    {{ t('itemLifecycleView.eventsTimeline') }}
                  </div>
                  <div 
                    v-for="event in lifecycle.events" 
                    :key="event.id"
                    class="flex items-center space-x-3 py-2 px-3 bg-[rgba(255,253,247,0.92)] rounded border"
                  >
                    <component 
                      ...
                      :class="{
                        ...
                        'text-tide-blue-deep': event.eventType === 'INTERMEDIATE'
                      }"
                    />
                    <div class="flex-1 min-w-0">
                      ...
                      <p v-if="event.notes" class="text-xs text-tide-ink/70 mt-1 truncate">
                        {{ event.notes }}
                      </p>
                      <p v-if="event.location" class="text-xs text-tide-ink/55">
    ...
    <div v-if="filteredLifecycles.length > 0" class="bg-[rgba(252,247,238,0.55)] rounded-lg p-4">
      <h4 class="text-sm font-medium text-tide-ink mb-3">{{ t('itemLifecycleView.summary') }}</h4>
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
        <div class="text-center">
          <div class="text-lg font-semibold text-tide-ink">
            {{ filteredLifecycles.length }}
          </div>
          <div class="text-xs text-tide-ink/55">{{ t('itemLifecycleView.stat.totalLifecycles') }}</div>
        </div>
        <div class="text-center">
          <div class="text-lg font-semibold text-tide-ink">
            {{ filteredLifecycles.filter(lc => lc.status === 'Completed').length }}
          </div>
          <div class="text-xs text-tide-ink/55">{{ t('itemLifecycleView.stat.completed') }}</div>
        </div>
        <div class="text-center">
          <div class="text-lg font-semibold text-tide-ink">
            {{ filteredLifecycles.filter(lc => lc.status === 'In Progress').length }}
          </div>
          <div class="text-xs text-tide-ink/55">{{ t('itemLifecycleView.stat.inProgress') }}</div>
        </div>
        <div class="text-center">
          <div class="text-lg font-semibold text-tide-ink">
            {{ formatDuration({ 
          ...
          <div class="text-xs text-tide-ink/55">{{ t('itemLifecycleView.stat.avgDuration') }}</div>
        </div>
      </div>
    </div>
  </div>
