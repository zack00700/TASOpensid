...
<template>
  <div class="min-h-screen bg-[rgba(252,247,238,0.55)]">
    ...
    <div class="px-6 py-6 space-y-6">
      ...
      <div class="bg-[rgba(255,253,247,0.92)] shadow-sm rounded-lg border border-[rgba(60,50,35,0.12)] overflow-hidden">
          ...
          <table class="min-w-full divide-y divide-[rgba(42,36,30,0.08)]">
            ...
            <tbody class="bg-[rgba(255,253,247,0.92)] divide-y divide-[rgba(42,36,30,0.08)]">
              ...
              >
                <!-- Reference -->
                <td class="px-4 py-3">
                  <span class="text-sm font-mono text-tide-ink">{{ payment.paymentReference || '—' }}</span>
                </td>

                <!-- Customer -->
                <td class="px-4 py-3">
                  <span class="text-sm font-medium text-tide-ink">{{ payment.customerName }}</span>
                </td>

                <!-- Amount -->
                <td class="px-4 py-3">
                  <span class="text-sm text-tide-ink font-medium">{{ formatAmount(payment.amount, payment.currency) }}</span>
                </td>

                <!-- Method -->
                <td class="px-4 py-3">
                  <span class="text-sm text-tide-ink/80">{{ formatMethodLabel(payment.paymentMethod) }}</span>
                </td>

                <!-- Payment Date -->
                <td class="px-4 py-3">
                  <span class="text-sm text-tide-ink/55 whitespace-nowrap">{{ formatDate(payment.paymentDate) }}</span>
                </td>
                ...
                <!-- Unallocated -->
                <td class="px-4 py-3">
                  <span class="text-sm text-tide-ink/80">
        ...
        <div class="sm:hidden p-4">
          ...
          <div v-else-if="!payments.length" class="bg-[rgba(255,253,247,0.92)] rounded-xl border border-[rgba(60,50,35,0.12)] p-8 text-center">
            <p class="text-sm text-tide-ink/55">{{ t('payments.empty.noPaymentsFound') }}</p>
          </div>
          <div v-else class="space-y-3">
            ...
            >
              <div class="flex items-start justify-between gap-2 mb-2">
                <div>
                  <p class="font-semibold text-tide-ink text-sm">{{ payment.customerName }}</p>
                  <p class="text-xs text-tide-ink/55 mt-0.5 font-mono">{{ payment.paymentReference || '—' }}</p>
                </div>
                <span :class="getStatusBadgeClasses(payment.status)">{{ payment.status ?? '—' }}</span>
              </div>
              <div class="flex items-center justify-between text-xs text-tide-ink/55 mt-1">
                <span class="font-medium text-tide-ink/80">{{ formatAmount(payment.amount, payment.currency) }}</span>
        ...
        <div v-if="!isLoading && payments.length === 0" class="hidden sm:block text-center py-12">
          <CreditCard class="mx-auto h-12 w-12 text-tide-ink/40" />
          <h3 class="mt-2 text-sm font-medium text-tide-ink">{{ t('payments.empty.title') }}</h3>
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-[rgba(255,253,247,0.92)] rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
          ...
          <div class="flex items-center justify-between p-6 border-b border-[rgba(60,50,35,0.12)]">
            ...
            <button @click="closeFormModal" class="p-2 text-tide-ink/40 hover:text-tide-ink/70 hover:bg-[rgba(42,36,30,0.05)] rounded-lg transition-colors">
              <X class="h-5 w-5" />
            </button>
          </div>
          ...
          <div class="overflow-y-auto flex-1 p-6 space-y-4">
            ...
            <!-- Notes -->
            <div>
              <label class="block text-sm font-medium text-tide-ink/80 mb-1">{{ t('payments.form.notes') }}</label>
              <textarea
                v-model="formData.notes"
                rows="3"
                :placeholder="t('payments.form.notesPlaceholder')"
                class="block w-full border border-[rgba(60,50,35,0.16)] rounded-lg py-2.5 px-3 text-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-tide-blue focus:border-tide-blue resize-none"
              ></textarea>
            </div>
          </div>
        ...
        <div class="bg-[rgba(255,253,247,0.92)] rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
          ...
          <div class="flex items-center justify-between p-6 border-b border-[rgba(60,50,35,0.12)]">
            <div class="flex items-center space-x-3">
              <CreditCard class="h-6 w-6 text-tide-blue-deep" />
              <div>
                <h3 class="text-lg font-semibold text-tide-ink">{{ t('payments.modal.detail.title') }}</h3>
                <p class="text-xs text-tide-ink/55 font-mono mt-0.5">{{ selectedPayment.id }}</p>
              </div>
            </div>
          ...
          <div class="overflow-y-auto flex-1 p-6 space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <div>
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.reference') }}</p>
                <p class="mt-1 text-sm font-mono text-tide-ink">{{ selectedPayment.paymentReference || '—' }}</p>
              </div>
              <div>
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.customer') }}</p>
                <p class="mt-1 text-sm font-medium text-tide-ink">{{ selectedPayment.customerName }}</p>
              </div>
              <div>
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.status') }}</p>
                <div class="mt-1">
                  <span :class="getStatusBadgeClasses(selectedPayment.status)">{{ selectedPayment.status ?? '—' }}</span>
                </div>
              </div>
              <div>
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.amount') }}</p>
                <p class="mt-1 text-sm font-medium text-tide-ink">{{ formatAmount(selectedPayment.amount, selectedPayment.currency) }}</p>
              </div>
              <div>
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.unallocated') }}</p>
              ...
              <div>
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.method') }}</p>
                <p class="mt-1 text-sm text-tide-ink">{{ formatMethodLabel(selectedPayment.paymentMethod) }}</p>
              </div>
              <div>
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.paymentDate') }}</p>
                <p class="mt-1 text-sm text-tide-ink">{{ formatDate(selectedPayment.paymentDate) }}</p>
              </div>
              <div>
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.receivedDate') }}</p>
                <p class="mt-1 text-sm text-tide-ink">{{ formatDate(selectedPayment.receivedDate) }}</p>
              </div>
              <div>
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.bankReference') }}</p>
                <p class="mt-1 text-sm font-mono text-tide-ink">{{ selectedPayment.bankReference || '—' }}</p>
              </div>
              <div v-if="selectedPayment.checkNumber">
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.checkNumber') }}</p>
                <p class="mt-1 text-sm font-mono text-tide-ink">{{ selectedPayment.checkNumber }}</p>
              </div>
              <div v-if="selectedPayment.processedBy">
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.processedBy') }}</p>
                <p class="mt-1 text-sm text-tide-ink">{{ selectedPayment.processedBy }}</p>
              </div>
            </div>

            <!-- Notes -->
            <div v-if="selectedPayment.notes">
              <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide mb-1">{{ t('payments.field.notes') }}</p>
            ...
            <div v-if="selectedPayment.reversalReason">
              <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide mb-1">{{ t('payments.field.reversalReason') }}</p>
            ...
            <div>
              <div class="flex items-center justify-between mb-3">
                <p class="text-xs font-medium text-tide-ink/55 uppercase tracking-wide">{{ t('payments.field.allocations') }}</p>
                <button
                  @click="showAllocationForm = !showAllocationForm"
                  class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-lg text-white bg-tide-blue-btn-deep hover:bg-tide-blue-deep"
                >
              ...
              <!-- Allocation form -->
              <div v-if="showAllocationForm" class="mb-4 bg-[rgba(90,138,171,0.10)] border border-blue-200 rounded-lg p-4 space-y-3">
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    <label class="block text-xs font-medium text-tide-ink/80 mb-1">{{ t('payments.form.invoiceId') }} <span class="text-red-500">*</span></label>
                    <input
                      v-model="allocationForm.invoiceId"
                      type="text"
                      :placeholder="t('payments.form.invoiceIdPlaceholder')"
                      class="block w-full border border-[rgba(60,50,35,0.16)] rounded-lg py-2 px-3 text-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-tide-blue focus:border-tide-blue"
                    />
                  </div>
                  <div>
                    <label class="block text-xs font-medium text-tide-ink/80 mb-1">{{ t('payments.form.amount') }} <span class="text-red-500">*</span></label>
                    <input
                ...
                <div class="flex justify-end space-x-2">
                  <button
                    ...
                    class="px-3 py-1.5 border border-[rgba(60,50,35,0.16)] rounded-lg text-xs font-medium text-tide-ink/80 hover:bg-[rgba(252,247,238,0.55)]"
                  >
                    {{ t('common.cancel') }}
                  </button>
                  <button
                    ...
                    class="inline-flex items-center px-3 py-1.5 border border-transparent rounded-lg text-xs font-medium text-white bg-tide-blue-btn-deep hover:bg-tide-blue-deep disabled:opacity-50"
                  >
              ...
              <!-- Allocations table -->
              <div v-if="selectedPayment.allocations?.length" class="border border-[rgba(60,50,35,0.12)] rounded-lg overflow-hidden">
                <table class="min-w-full divide-y divide-[rgba(42,36,30,0.08)]">
                  <thead class="bg-[rgba(252,247,238,0.55)]">
                    <tr>
                      <th class="px-3 py-2 text-left text-xs font-medium text-tide-ink/55 uppercase">{{ t('payments.column.alloc.invoiceId') }}</th>
                      <th class="px-3 py-2 text-left text-xs font-medium text-tide-ink/55 uppercase">{{ t('payments.column.alloc.invoiceNumber') }}</th>
                      <th class="px-3 py-2 text-left text-xs font-medium text-tide-ink/55 uppercase">{{ t('payments.column.alloc.amount') }}</th>
                      <th class="px-3 py-2 text-left text-xs font-medium text-tide-ink/55 uppercase">{{ t('payments.column.alloc.date') }}</th>
                    </tr>
                  </thead>
                  <tbody class="bg-[rgba(255,253,247,0.92)] divide-y divide-[rgba(42,36,30,0.08)]">
                    <tr v-for="alloc in selectedPayment.allocations" :key="alloc.allocationId ?? alloc.invoiceId">
                      <td class="px-3 py-2 text-xs font-mono text-tide-ink">{{ alloc.invoiceId }}</td>
                      <td class="px-3 py-2 text-xs text-tide-ink/80">{{ alloc.invoiceNumber || '—' }}</td>
                      <td class="px-3 py-2 text-xs text-tide-ink font-medium">{{ formatAmount(alloc.allocatedAmount, selectedPayment.currency) }}</td>
                      <td class="px-3 py-2 text-xs text-tide-ink/55">{{ formatDate(alloc.allocationDate) }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              <div v-else class="text-center py-6 bg-[rgba(252,247,238,0.55)] rounded-lg border border-[rgba(60,50,35,0.12)]">
                <p class="text-sm text-tide-ink/55">{{ t('payments.empty.noAllocations') }}</p>
              </div>
            </div>
          </div>
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-[rgba(255,253,247,0.92)] rounded-xl shadow-xl w-full max-w-md">
          <div class="p-6 border-b border-[rgba(60,50,35,0.12)]">
            <h3 class="text-lg font-semibold text-tide-ink">{{ t('payments.modal.reverse.title') }}</h3>
            <p class="text-sm text-tide-ink/55 mt-0.5">{{ t('payments.modal.reverse.subtitle') }}</p>
          </div>
          <div class="p-6">
            <label class="block text-sm font-medium text-tide-ink/80 mb-1">{{ t('payments.field.reversalReason') }} <span class="text-red-500">*</span></label>
            <textarea
