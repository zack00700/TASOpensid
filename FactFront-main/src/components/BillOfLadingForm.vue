...
/* ---------------------- Attach existing item (TC-14) ---------------------- */
// Opens a small picker that lets the user search a previously-created item and
// attach it to the current BL. Avoids having to retype container numbers when
// the item already exists in the yard.
const showAttachItem = ref(false);
const attachItemQuery = ref("");
const attachItemResults = ref<any[]>([]);
const attachItemLoading = ref(false);

let attachItemDebounce: ReturnType<typeof setTimeout> | null = null;
watch(attachItemQuery, (q) => {
  if (attachItemDebounce) clearTimeout(attachItemDebounce);
  attachItemDebounce = setTimeout(() => searchAttachableItems(q), 250);
});

async function searchAttachableItems(q: string) {
  const term = (q || "").trim();
  if (term.length < 2) {
    attachItemResults.value = [];
    return;
  }
  attachItemLoading.value = true;
  try {
    // itemService.getItemsPaginated exposes a `search` filter on the back.
    const res = await itemService.searchItems(term);
    const existingIds = new Set(formData.value.items.map((i: any) => i.id));
    attachItemResults.value = (res ?? []).filter((it: any) => !existingIds.has(it.id));
  } catch (e) {
    console.error("Failed to search items", e);
    attachItemResults.value = [];
  } finally {
    attachItemLoading.value = false;
  }
}

function openAttachItem() {
  attachItemQuery.value = "";
  attachItemResults.value = [];
  showAttachItem.value = true;
}

function attachExistingItem(item: any) {
  if (!item?.id) return;
  if (formData.value.items.some((i: any) => i.id === item.id)) {
    showAttachItem.value = false;
    return;
  }
  // Map the back's Item shape into the form's local Item shape — keep the
  // original id so the back BL create reuses (not re-creates) the document.
  formData.value.items.push({
    clientId: item.id,
    id: item.id,
    itemType: item.itemType ?? "container",
    type: item.type ?? "",
    ...
    relatedInvoice: item.relatedInvoice ?? null,
    billOfLadingId: blId.value ?? null,
    expanded: false,
    errors: {},
  });
  showAttachItem.value = false;
...
<div class="bg-white border-b border-gray-200">
  <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center">
              ...
              <div class="flex items-center gap-2">
                ...
                >
                  ...
                  {{ t("billOfLadingForm.button.attachExistingItem") }}
                </button>
                <button
                  @click="addItem"
    ...
    <!-- Attach existing item modal (TC-14) -->
    <Teleport to="body">
      <div
        v-if="showAttachItem"
