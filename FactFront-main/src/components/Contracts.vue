...
const getStatusBadgeClasses = (status: string) => {
  ...
  switch (status) {
    ...
    case 'Disable':
      return `${baseClasses} bg-[rgba(42,36,30,0.05)] text-tide-ink border border-[rgba(60,50,35,0.12)]`;
    default:
      return baseClasses;
  }
};
...
const getEventTypeClasses = (type: string | undefined) => {
  ...
  switch (type) {
    ...
    case 'INTERMEDIATE': return `${baseClasses} text-tide-blue-deep`;
    ...
    <div v-if="!showForm">
      ...
      >
        <template #actions>
          <Button
            variant="secondary"
            :class="showAdvancedFilter && 'bg-[rgba(90,138,171,0.10)] text-tide-blue-deep border-blue-300'"
      ...
      <div class="bg-[rgba(255,253,247,0.92)] rounded-lg shadow-sm border border-[rgba(60,50,35,0.12)] mb-6">
        <div class="p-6">
          <div class="flex flex-col sm:flex-row gap-4">
            ...
            <div class="flex items-center space-x-2">
              <span class="text-sm text-tide-ink/55">{{ t('contracts.label.viewLabel') }}</span>
              <div class="flex rounded-lg border border-[rgba(60,50,35,0.12)] p-1">
                <button
                  ...
                  :class="[
                    ...
                    viewMode === 'table'
                      ...
                      : 'text-tide-ink/55 hover:text-tide-ink/80'
                  ]"
                >
        ...
        <div v-if="showAdvancedFilter" class="border-t border-[rgba(60,50,35,0.12)] px-4 py-3 bg-[rgba(252,247,238,0.55)]">
          <AdvancedFilter type="contracts" @filter="handleFilter" />
        </div>
      </div>

      <!-- Bulk Actions Bar -->
      <div v-if="selectedContracts.length > 0" class="bg-[rgba(90,138,171,0.10)] border border-blue-200 rounded-lg p-4 mb-6">
      ...
      <div class="bg-[rgba(255,253,247,0.92)] shadow-sm rounded-lg border border-[rgba(60,50,35,0.12)] overflow-hidden">
          ...
          <table class="min-w-full divide-y divide-[rgba(42,36,30,0.08)]">
            <thead class="bg-[rgba(252,247,238,0.55)]">
              <tr>
                ...
                <th v-for="header in tableHeaders" :key="header" class="px-4 py-3 text-left text-xs font-medium text-tide-ink/55 uppercase tracking-wider">
                  {{ header }}
                </th>
              </tr>
            </thead>
            <tbody class="bg-[rgba(255,253,247,0.92)] divide-y divide-[rgba(42,36,30,0.08)]">
              <tr 
                ...
                :key="contract.id" 
                :class="[
                  'hover:bg-[rgba(252,247,238,0.55)] transition-colors duration-150',
                  selectedContracts.includes(contract.id) ? 'bg-[rgba(90,138,171,0.10)]' : ''
                ]"
              >
                <td class="px-4 py-3">
                  <input
                ...
                <td class="px-4 py-3">
                  <div class="max-w-xs">
                    <div class="text-sm font-semibold text-tide-ink truncate">
                      {{ contract.name }}
                    </div>
                    <div class="text-sm text-tide-ink/55 truncate">
                ...
                <td class="px-4 py-3">
                  <div :class="getEventTypeClasses(contract.calculationMode.eventConfig?.eventType)">
                    ...
                    />
                    <div class="min-w-0">
                      <div class="text-sm font-medium text-tide-ink truncate">
                        {{ contract.calculationMode.eventConfig?.eventName }}
                      </div>
                      <div class="text-xs text-tide-ink/55 capitalize">
                ...
                <td class="px-4 py-3">
                  <div class="text-sm">
                    <div class="font-medium text-tide-ink">{{ contract.calculationMode.type }}</div>
                    <div class="text-tide-ink/55 capitalize">
        ...
        <div class="sm:hidden p-4">
          <div v-if="!filteredContracts || filteredContracts.length === 0" class="bg-[rgba(255,253,247,0.92)] rounded-xl border border-[rgba(60,50,35,0.12)] p-8 text-center">
            <p class="text-sm text-tide-ink/55">{{ t('contracts.empty.noContracts') }}</p>
          </div>
          <div v-else class="space-y-3">
            ...
            >
              <div class="flex items-start justify-between gap-2 mb-2">
                <div>
                  <p class="font-semibold text-tide-ink text-sm">{{ contract.name }}</p>
                  <p class="text-xs text-tide-ink/55 mt-0.5">{{ contract.description || '—' }}</p>
                </div>
                <span v-if="contract.status" :class="[
                  'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border shrink-0',
                  contract.status === 'Active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-[rgba(42,36,30,0.05)] text-tide-ink/70 border-[rgba(60,50,35,0.12)]'
        ...
        <div v-else class="p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <div
              v-for="contract in filteredContracts"
              :key="contract.id"
              class="bg-[rgba(255,253,247,0.92)] border border-[rgba(60,50,35,0.12)] rounded-lg hover:shadow-md transition-shadow duration-200 overflow-hidden"
            >
              <div class="p-5">
                <div class="flex items-start justify-between mb-3">
                  <div class="flex-1 min-w-0">
                    <h3 class="text-sm font-semibold text-tide-ink truncate">
                      {{ contract.name }}
                    </h3>
                    <p class="text-xs text-tide-ink/55 mt-1 line-clamp-2">
                      {{ contract.description }}
                    </p>
                  </div>
                  <span :class="getStatusBadgeClasses(contract.status)">
                ...
                <div class="flex justify-between items-center mt-4 pt-3 border-t border-[rgba(42,36,30,0.07)]">
        ...
        <div v-if="filteredContracts.length === 0" class="text-center py-12">
          <Settings class="mx-auto h-12 w-12 text-tide-ink/40" />
        ...
        <div class="bg-[rgba(255,253,247,0.92)] rounded-xl shadow-xl max-w-md w-full">
          <div class="p-6">
            <div class="flex items-center space-x-3">
              ...
              <div class="flex-1">
                <h3 class="text-lg font-semibold text-tide-ink">{{ t('contracts.deleteModal.title') }}</h3>
