...
public class ItemDaoImpl implements ItemDao {
     ...
     *  Items that already have an explicit stored status (set via PUT) keep it —
     *  mirrors {@link #getItem(String)} so list and detail views agree. */
    private void applyStatusBatch(List<Item> items) {
        if (items.isEmpty()) return;
        Map<String, Lifecycle> cache = buildLifecycleCache(items);
        items.forEach(i -> {
            if (!i.hasStoredStatus()) {
    ...
    public Item getItem(String itemId) {
        ...
        if (item != null && !item.hasStoredStatus()) {
            // Default behaviour: derive from the lifecycle. An explicitly stored
            // status (persisted via PUT) is preserved (CLAUDE_back TC-13).
