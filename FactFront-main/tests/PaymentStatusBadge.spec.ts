...
describe('PaymentStatusBadge', () => {
  it('renders the Paid label and emerald palette', () => {
    ...
    expect(cls).toMatch(/bg-\[rgba\(101,153,123/);
    expect(cls).toMatch(/text-tide-green-deep/);
  });
  ...
  it('renders the Partial label and blue palette', () => {
    ...
    expect(w.find('span').attributes('class') || '').toMatch(/bg-\[rgba\(90,138,171/);
  });
  ...
  it('renders the Unpaid label and slate palette', () => {
    ...
    expect(w.find('span').attributes('class') || '').toMatch(/bg-\[rgba\(42,36,30/);
  });
  ...
  it('renders the Overdue label and red palette', () => {
    ...
    expect(w.find('span').attributes('class') || '').toMatch(/bg-\[rgba\(181,99,88/);
    expect(w.find('span').attributes('class') || '').toMatch(/text-tide-rust-deep/);
  });
});
